...
- `Embedder.embed(texts)` — the main entry point. For each input text, computes a content hash via `Chunk.make_content_hash(model, text)` — a type-safe static method returning a `str` — looks up each unique hash via the type-safe `chunk_store.find_by_hash()` (returns `list[float] | None`), and only calls the OpenRouter API for cache misses, in batches of `batch_size`. **Writes no cache entries** — read-only; persistence is the ingest script's responsibility. Returns one vector per input text, **in original input order** (not batch/cache order). On batch failure, logs the failed texts and error to the DLQ via `chunk_store.add_failed()` (typed `text: str, error: str, model: str, content_hash: str | None = None`), then re-raises the error.
...
- `Embedder.embed_documents(texts)` / `Embedder.embed_query(text)` — `langchain_core.embeddings.Embeddings` interface methods (thin delegates to `embed`/`embed_one`). `Embedder` now subclasses `Embeddings` so the same instance can be wrapped by `ragas.embeddings.LangchainEmbeddingsWrapper` for [RAGAS eval scoring](ragas_adapters.md), without a second embeddings implementation.
...
- [RAGAS Adapters](/doc/feature/ragas_adapters.md) — reuses this class (via its `Embeddings` interface) as the RAGAS judge embeddings
