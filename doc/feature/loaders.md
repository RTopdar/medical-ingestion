...
- `pdf.py` — `PDFLoaderService`, wraps Docling's `DocumentConverter` and `HybridChunker`. Converts each PDF via `DocumentConverter().convert()` (single pass, no re-parsing), then chunks the parsed document directly with `HybridChunker` — avoids the double-parse overhead of wrapping `DoclingLoader`. Extracts hierarchical section path (ancestor chain across all heading levels, e.g., "Part > Chapter > Section") once per document via `_extract_section_path_from_document()` and applies it to every chunk. The full hierarchical path is stored in `metadata["section_path"]` (list of strings); `metadata["section"]` is a flattened display format. Also populates `page_number` from chunk provenance. Metadata dict includes: `source`, `source_type`, `title`, `tags`, `page_number`, `element_type`, `section`, `section_path`, `bbox`, `char_span`, `content_layer`.
