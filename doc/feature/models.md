...
Rewritten for the Postgres migration (SQLite `EmbeddingCache`/`DocumentCache` → Postgres). Plain-Pydantic `Vector` model removed (superseded by `Chunk`, below). **All ORM classes now provide type safety + IDE autocomplete** — every field is typed, queries use SQLAlchemy's `select()` API with typed column references, and ORM methods like `order_by()` operate on fully-typed fields (e.g., `order_by(desc(FailedEmbedding.created_at))` instead of string-based sorts).

- `Chunk` — `SQLModel(table=True)`, table `chunks`. One row per chunk **occurrence** — the same `content_hash` intentionally repeats across rows when identical text appears in multiple documents/patients, so `content_hash` is indexed but **not unique**. Fields: `id` (PK), `content_hash`, `text`, `model`, `embedding` (`list[float]`, JSON column — cache-lookup only, never searched), `metadata_` (dict, JSON column aliased `"metadata"` — carries `source`, `source_type`, `patient_mrn`, `document_id`, etc.), `section_path` (`Optional[list[str]]`, JSON column — hierarchical section path from document structure, e.g., `['Introduction', 'Background', 'Clinical Context']`, null if no heading structure available; added 2026-09-02 via Alembic 32cedf430be3), `page_number` (`Optional[int]` — PDF page metadata for multi-page document retrieval, null if not available; added 2026-09-02 via Alembic 32cedf430be3), `document_content_hash` (optional FK to `documents.content_hash`), `created_at` (datetime with default factory). **New fields (Task 1 of 5, structured chunk metadata):** `section_path` and `page_number` enable structure-aware retrieval workflows — chunks can be filtered/reranked by section hierarchy and page location, improving retrieval precision for multi-section medical documents and supporting multi-level context preservation in RAG pipelines. `Chunk.make_content_hash(model, text)` — `staticmethod`, `sha256(f"{model}:{normalized_text}")`, same logic as the old `EmbeddingCache.make_key`. **Strongly typed field references:** queries like `select(Chunk).where(Chunk.content_hash == "abc123")` receive full IDE autocomplete on column names and types.
- `FailedEmbedding` — `SQLModel(table=True)`, table `failed_embeddings`. DLQ, same shape as the old sqlite DLQ table (`content_hash`, `text`, `error`, `model`, `attempt_count`, `created_at`). Used by [Chunk Store](/doc/feature/chunk_store.md) for failure tracking; queries like `select(FailedEmbedding).order_by(desc(FailedEmbedding.created_at))` (note: uses `sqlalchemy.desc()` function for type-safe sorting, not `.desc()` method on the field itself).
- `IngestedDocument` — `SQLModel(table=True)`, table `documents`. `content_hash` primary key, `source`, `ingested_at`. Whole-document dedup gate, replaces the deleted `DocumentCache`. `IngestedDocument.make_content_hash(content)` — `staticmethod`. Queries check existence before inserting via `select(IngestedDocument).where(IngestedDocument.content_hash == "abc123")`.
