...
First smoke-tested run (`--limit 2`, default `chat_model=openrouter/free` as judge, 2026-09-12): `faithfulness=1.0`, `answer_relevancy≈0.96`, `context_recall=1.0` scored cleanly; `llm_context_precision_with_reference` and `answer_correctness` hit `TimeoutError` on the free-tier judge model under concurrent RAGAS sub-calls. This is not a pipeline defect — it's the free-tier judge model's rate limits — and confirms the recommendation above to set `RAGAS_JUDGE_MODEL` for real runs. Scores are meaningless without a comparison point; treat this only as confirmation the integration loop works end-to-end, not as a quality baseline (re-run with a stable judge model before using scores for regression tracking).
