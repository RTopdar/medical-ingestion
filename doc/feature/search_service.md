...
- `SearchService.__init__()` — constructs a `BM25Index` and loads it from disk (`settings.bm25_index_path`), a `HybridRetriever(QdrantVectorStore(), bm25_index)`, a `Reranker()`, and one `self.chat_router = ChatRouterService.from_settings(settings)` (see [Chat Router](chat_router.md)) — built once, reused across queries. Fails loudly if the BM25 index hasn't been built yet (`scripts/ingest_documents.py` must run first).
...
- `answer(query, chunks, citations=None)` — generator, streams a grounded LLM response via `self.chat_router.stream(...)` (OpenRouter -> Groq -> OpenRouter fallback chain, see [Chat Router](chat_router.md); previously constructed `ChatOpenRouter` directly per call). Chunks are numbered `[N]` in the prompt (renumbered from `[Chunk N]`) and, when `citations` is passed, each chunk gets an appended `[Source: ...]` marker. Prompt instructs the model to cite inline using `[1]`, `[2]`, etc. `citations` param is optional/backward-compatible — omitting it just drops the source markers, chunk numbering still happens. Public generator behavior (yields string chunks) is unchanged by the router migration.
...
- [Chat Router](chat_router.md) — shared litellm fallback chain this module's `answer()` streams through
