#!/usr/bin/env python3
"""Interactive hybrid search demo — dense + sparse fused via RRF, top-5 chunks with sources."""
...
def search_hybrid(retriever: HybridRetriever, query: str, top_k: int = 5) -> None:
    """Search dense+sparse fused results, display them, and stream LLM response."""
    query_embedding = embed_query(query)
    results = retriever.search(query, query_embedding, top_k=top_k)
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"\n📌 Result #{i} (RRF Score: {result['score']:.4f})")
...
def main():
    """Main REPL loop."""
    print("\n🔍 Medical Ingestion Hybrid Search Demo")
    print("=" * 80)
    print("Enter questions to search through ingested chunks (dense + sparse fused).")
    ...
    retriever = HybridRetriever(QdrantVectorStore(), bm25_index)
